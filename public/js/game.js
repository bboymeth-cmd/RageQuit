...
// CACHED GEOMETRY & MATERIALS (Performance Fix)
const HEAL_GEO = new THREE.CylinderGeometry(5, 5, 24, 32, 1, true);
const HEAL_MAT_PFX = new THREE.MeshBasicMaterial({
    color: 0x00ff00,
    transparent: true,
    opacity: 0.5,
...
function spawnHealCylinder(targetMesh) {
    ...
    // REUSE GEOMETRY - CLONE MATERIAL for independent fading
    const material = HEAL_MAT_PFX.clone();
    const cylinder = new THREE.Mesh(HEAL_GEO, material);
    ...
    // Light for extra glow (Optimized)
    const light = new THREE.PointLight(0x00ff00, 2, 15);
    light.castShadow = false; // PERFORMANCE FIX
    ...
    const animInterval = setInterval(() => {
        ...
        if (duration > 1000 || cylinder.material.opacity <= 0) {
            ...
            scene.remove(light);
            // DO NOT DISPOSE GLOBAL GEOMETRY
            material.dispose();
        }
    }, 16);
}
...
function init() {
    ...
    // VISUAL UPGRADE: Volumetric-style Fog (Cyan/Violaceous for Dark Fantasy depth)
    // Blend of deep purple and cyan
    // Blend of deep purple and cyan
    // Lighter Purple/Blue for sky
    const fogColor = new THREE.Color(0x150525);
    scene.background = fogColor;
    scene.fog = new THREE.FogExp2(0x150525, 0.0009);

    // FIX: Z-Fighting (Flickering) VS FPS Clipping
    // Reverted to 0.01 because 0.20 clipped the FPS Archer model.
    // We accept some Z-fighting on distant objects to ensure the player's weapon is visible.
    ...
    // VISUAL UPGRADE: LIGHTING & SHADOWS
    // 1. Ambient Light (Moonlight Scatter) - Low intensity, slight blue tint
    // VISUAL UPGRADE: LIGHTING & SHADOWS
    // 1. Ambient Light (Neutral Grey - Desaturated)
    // Removed blue tint to lower saturation. Keeping intensity 0.6.
    const ambientLight = new THREE.AmbientLight(0x404040, 2.0);
    scene.add(ambientLight);

    // 2. The Moon (Directional Light - Pure White)
    // Changed from Blue (0xaaccff) to White (0xffffff) to reduce saturation.
    const moonLight = new THREE.DirectionalLight(0xffffff, 4.0);

    // COORDINATE UPDATE: Center on Map (-7773, 655, -2922)
    const mapCenter = new THREE.Vector3(-7773, 655, -2922);

    // Position: High up and angled relative to map center
    // CUSTOM: Lower Y (4000 -> 1200) for Longer "Bigger" Shadows per user request
    moonLight.position.copy(mapCenter).add(new THREE.Vector3(2000, 1200, 2000));
    moonLight.target.position.copy(mapCenter);
    scene.add(moonLight.target); // IMPORTANT: Add target to scene
    ...
    scene.add(moonLight);
    // scene.add(new THREE.CameraHelper(moonLight.shadow.camera)); // Debug Shadows

    // CUSTOM USER REQUEST: Powerful Torch Light at Map Center
    const torchPos = new THREE.Vector3(-7773, 1000, -2922);
    const centralTorch = new THREE.PointLight(0xffaa00, 10.0, 5000, 1); // Intensity 10.0, Range 5000
    centralTorch.position.copy(torchPos);
    centralTorch.castShadow = false; // DISABLED SHADOWS per user request (Performance)
    centralTorch.shadow.bias = -0.001;
    scene.add(centralTorch);

    // Visual Sphere HIDDEN per user request
    /*
    const torchGeo = new THREE.SphereGeometry(20, 16, 16);
    ...
    */
    ...
    renderer = new THREE.WebGLRenderer({
        ...
        precision: 'highp', // Improved precision
        logarithmicDepthBuffer: true, // FIX: Solves Z-Fighting for huge distances (0.01 to 30000)
        alpha: false,
        stencil: false,
        depth: true
    });
    ...
    window.addEventListener('resize', () => {
        ...
        renderer.setSize(window.innerWidth, window.innerHeight);
        if (window.composer) window.composer.setSize(window.innerWidth, window.innerHeight);

        // Update FXAA Resolution
        if (window.fxaaPass) {
            const pixelRatio = renderer.getPixelRatio();
            window.fxaaPass.material.uniforms['resolution'].value.x = 1 / (window.innerWidth * pixelRatio);
            window.fxaaPass.material.uniforms['resolution'].value.y = 1 / (window.innerHeight * pixelRatio);
        }
    });
    ...
    try {
        if (typeof THREE.EffectComposer !== 'undefined') {
            ...
            // 1. SSAO (Screen Space Ambient Occlusion) - Darkens corners/contacts
            if (typeof THREE.SSAOPass !== 'undefined') {
                const ssaoPass = new THREE.SSAOPass(scene, camera, window.innerWidth, window.innerHeight);
                ssaoPass.kernelRadius = 4; // Reduced radius
                ssaoPass.minDistance = 0.04; // Increased to exclude FPS weapon (Archer is very close)
                ssaoPass.maxDistance = 0.1;
                window.composer.addPass(ssaoPass);
                console.log('[GRAPHICS] SSAO Enabled');
            } else {
                console.warn('[GRAPHICS] THREE.SSAOPass not found.');
            }

            // 2. BLOOM (Glow)
            // UnrealBloomPass(resolution, strength, radius, threshold)
            // Strength: 1.5 (Balanced), Radius: 0.8, Threshold: 0.8 (Only very bright hits glow)
            ...
            // 3. FXAA (Anti-Aliasing) - Must be last!
            if (typeof THREE.FXAAShader !== 'undefined') {
                window.fxaaPass = new THREE.ShaderPass(THREE.FXAAShader);
                const pixelRatio = renderer.getPixelRatio();
                window.fxaaPass.material.uniforms['resolution'].value.x = 1 / (window.innerWidth * pixelRatio);
                window.fxaaPass.material.uniforms['resolution'].value.y = 1 / (window.innerHeight * pixelRatio);
                window.composer.addPass(window.fxaaPass);
                console.log('[GRAPHICS] FXAA Enabled');
            } else {
                console.warn('[GRAPHICS] FXAAShader not found.');
            }

            console.log('[GRAPHICS] Post-Processing & Bloom Enabled');
        } else {
            console.warn('[GRAPHICS] EffectComposer not loaded, skipping bloom.');
        }
    } catch (e) {
        console.error('[GRAPHICS] Error setting up composition:', e);
    }
...
function animate() {
    ...
    // CUSTOM: LIMIT MAX FPS TO 60 (Prevents 240Hz stuttering)
    if (time - prevTime < 16) return;
    ...
    frameCount++;
    // OPTIMIZATION: Light Culling (Lanterns)
    if (window.mapLights && window.player && window.player.position) {
        const cullDist = 200; // Reduced to 200 per user request for performance
        const cullDistSq = cullDist * cullDist;
        const playerPos = camera.position; // Use camera for culling (what player sees)

        // Throttling: Update every 10 frames (approx 160ms @ 60fps)
        if (frameCount % 10 === 0) {
            // 1. LIGHT CULLING
            for (let i = 0; i < window.mapLights.length; i++) {
                const light = window.mapLights[i];
            ...
            if (window.mapBuildings) {
                ...
                for (let i = 0; i < window.mapBuildings.length; i++) {
                    const building = window.mapBuildings[i];
                    // Building position is world position because map is centered at 0,0,0?? 
                    // No, map is scaled. But building.position is local to map.
                    // We need world position. accessing world position is slightly expensive but ok for 50 objects every 10 frames.
                    // Optimization: use approx distance if map is static? 
                    // Let's use World Position for correctness.
    ...
    const logicTime = t1 - t0;

    // RENDERER
    if (window.composer) {
        // window.composer.render(); // Called below
    } else {
        // renderer.render(scene, camera); // Called below
    }

    // NOTE: The original render call is below. 
    // We wrap it in measurement:
    ...
    // --- PROFILER AGGREGATE ---
    window.profFrames = (window.profFrames || 0) + 1;
    window.profLogic = (window.profLogic || 0) + logicTime;
    window.profRender = (window.profRender || 0) + renderTime;

    if (window.profFrames >= 300) { // Every ~5 seconds (at 60fps)
        console.group('[PERFORMANCE REPORT 5s]');
        console.table({
            'FPS': currentFPS,
            'Logic (CPU)': (window.profLogic / 300).toFixed(2) + 'ms',
            'Render (GPU)': (window.profRender / 300).toFixed(2) + 'ms',
            'Geometries': renderer.info.memory.geometries,
            'Textures': renderer.info.memory.textures,
            'Draw Calls': renderer.info.render.calls,
            'Triangles': renderer.info.render.triangles,
            'Active Lights': window.mapLights ? window.mapLights.filter(l => l.visible).length : 0,
            'Total Lights': window.mapLights ? window.mapLights.length : 0
        });
        console.groupEnd();
