...
function spawnStoneSpikes(target, isSticky = false) {
    ...
    // OPTIMIZATION: Use BasicMaterial (No light calc) instead of Lambert
    const material = new THREE.MeshBasicMaterial({ color: 0x888888 });
    for (let i = 0; i < 8; i++) {
        const height = 8 + random() * 4; const geo = new THREE.ConeGeometry(1.5, height, 6); const mesh = new THREE.Mesh(geo, material);
        mesh.castShadow = false; mesh.receiveShadow = false; // PERFORMANCE
...
function spawnProjectile(type) {
    ...
    proj.castShadow = false; // PERFORMANCE: No shadow casting
    proj.receiveShadow = false; // PERFORMANCE: No shadow receiving
    ...
    else { const light = new THREE.PointLight(color, 1, 30); light.castShadow = false; proj.add(light); }
...
function spawnEnemyProjectile(startPos, direction, type) {
    ...
    if (type === 5) {
        ...
        const light = new THREE.PointLight(color, 1, 20); light.castShadow = false; proj.add(light);
    }
...
function startCastingEffect(playerObj, spellId) {
    ...
    if (isEnhanced) {
        // Add Internal PointLight (Optimized)
