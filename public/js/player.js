...
let swordAnimationSpeed = 15; // FIX: Missing variable causing ReferenceError in loop
    ...
    loader.load('./models/archer.glb', (gltf) => {
        ...
        model.traverse((child) => {
            if (child.isMesh) {
                ...
                child.castShadow = false; // FIX: No casting shadows (removes lines)
                child.receiveShadow = false; // FIX: No receiving shadows (removes artifacts)
                child.renderOrder = 1; // FIX: Ensure it renders on top if needed, or consistent
                if (child.material) {
...
function loadMageModel() {
    ...
    loader.load('./models/mage.glb', (gltf) => {
        ...
        model.traverse((child) => {
            if (child.isMesh) {
                child.frustumCulled = false;
                child.castShadow = true; // FIX: Shadows enabled
                child.receiveShadow = true;
                if (child.material) {
                    // FIX: Rimuove DoubleSide che causa buchi nelle texture
                    child.material.side = THREE.FrontSide;
...
function spawnGlowEffect(color) {
    ...
    light.position.copy(spawnPos); scene.add(light);
    let intensity = 5;
...
function toggleWeapon(force) {
    ...
    if (isMelee) {
        ...
        if (knightModel) {
            ...
            // PERFORMANCE FIX: Removed redundant traverse color update. Color is set at load.
        }
    } else {
    ...
    const modeEl = document.getElementById('weapon-mode-text'); // Utilize cached ref if possible
    if (modeEl) {
    ...
    const slotQ = document.getElementById('slot-q'); if (slotQ) slotQ.classList.remove('active');
    const slotE = document.getElementById('slot-e'); if (slotE) slotE.classList.remove('active');

    if (isMelee || isRanged) {
        if (slotQ) slotQ.classList.add('active');
        // PERFORMANCE FIX: Removed camera.updateProjectionMatrix duplication
        // Camera Near is globally 0.01 now.
    } else if (isBow) {
...
function updatePhysics(delta) {
    ...
    if (horizVel.lengthSq() > 0.1) {
        ...
        // PROBE PHYSICS V7: Optimized Probes (Width + Height)
        // Reduced from 21 rays to 6 for performance.
        // Widths: Left Shoulder (-3), Right Shoulder (3). Center is implicit by body width? No, let's just check sides.
        // Actual widths: -2.5, 2.5
        const widthOffsets = [-2.5, 2.5];
        // Heights: Feet (-5), Waist (0), Head (8)
        const heightOffsets = [-5, 0, 8];
