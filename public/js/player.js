...
let knightAnimationMixer = null;
let swordAttackAction = null;
...
function loadKnightModel() {
            if (isLoadingModel || knightModel) return;
            isLoadingModel = true;
            
            const loader = new THREE.GLTFLoader();
            
            // Carica modello Knight Met.glb
            loader.load('./models/Knight Met.glb', (gltf) => {
                knightModel = gltf.scene;
                knightModel.scale.set(5, 5, 5); // Scala modello
                knightModel.visible = false; // Nascosto di default (visibile solo in melee)
                
                // Applica colore team al materiale
                ...
                // Carica animazione Sword Attack
                loader.load('./animations/Sword Attack 1.glb', (animGltf) => {
                    if (animGltf.animations && animGltf.animations.length > 0) {
                        knightAnimationMixer = new THREE.AnimationMixer(knightModel);
                        swordAttackAction = knightAnimationMixer.clipAction(animGltf.animations[0]);
                        swordAttackAction.setLoop(THREE.LoopOnce);
                        swordAttackAction.clampWhenFinished = true;
                        console.log('[SWORD ANIMATION] Loaded successfully');
                    }
                }, undefined, (error) => {
                    console.error('[SWORD ANIMATION] Load error:', error);
                });
                
                isLoadingModel = false;
            }, undefined, (error) => {
                console.error('[KNIGHT MODEL] Load error:', error);
                isLoadingModel = false;
            });
        }
...
function createPlayer() {
            ...
            // Carica modello Knight GLB in background
            loadKnightModel();
...
function updatePlayerColor() {
            ...
            // Aggiorna anche il modello Knight se caricato
            if (knightModel) {
                knightModel.traverse((child) => {
                    if (child.isMesh && child.material) {
                        child.material.color.setHex(currentColor);
...
function toggleWeapon(force) {
            ...
            swordContainer.visible = false; // Nascosto - il modello Knight ha già la spada
            staffContainer.visible = isRanged;
            bowContainer.visible = isBow;
            
            // Mostra modello Knight solo in melee
            if (knightModel) {
                knightModel.visible = isMelee;
            }
            
            playerMesh.visible = true; // Sempre visibile
            if (isRanged || isBow) {
                stopBlocking();
                euler.x = 0; 
            } else if (isMelee) {
            ...
            // Nascondi parti geometriche in melee (usa Knight model invece)
